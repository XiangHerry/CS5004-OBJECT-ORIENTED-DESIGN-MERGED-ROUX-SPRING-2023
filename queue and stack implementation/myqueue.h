...
int queue_empty(queue_t* q){
	// TODO: Implement me!
	// if two pointers are same and size as 0, we can return 1.
	if (q->front == q->back && q->size == 0) {
		return 1;
	} 
	// else, not empty.
	else {
...
int queue_full(queue_t* q){
	// TODO: Implement me!
	// if size is same to capacity, it is full.
...
int queue_enqueue(queue_t* q, int item){
	...
	if (queue_full(q) == 1) {
		...
		// add item in the end.
		// used to access the element of the "data" array at the index stored in the "back" variable.
		q->data[q->back] = item;
		// add the index for back by 1.
		q->back = (q->back + 1) % q->capacity;
		// size also increases by 1.
		q->size++;
		return 0;
...
int queue_dequeue(queue_t *q){
	// TODO: Implement me!
	if (queue_empty(q) == 1) {
		printf("Error detected! Trying to remove element from an empty queue!\n");
		exit(1);

	} else {
		// store the item in the front of the queue.
		int front_item = q->data[q->front];
		// move the position of the front element.
...
unsigned int queue_size(queue_t* q){
	// TODO: Implement me!
	// if q is null, we can exit.
	if (q == NULL) {
		exit(1);
	} 
	// else, we can return the size of queue.
	else {
		return q->size;
	}
	return 0;
...
void free_queue(queue_t* q){
	// TODO: Implement me!
	// if q is null, we can return. nothing to free
	if (q == NULL) {
		return;
	} 
	// else, we can free data of q as well as q.
	else {
		free(q->data);
		free(q);
	}
